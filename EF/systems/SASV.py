...
from utils.metrics import get_all_EERs
from utils.utils import keras_decay
from models.SASV import OCSoftmaxWithLoss


class System(pl.LightningModule):
    def __init__(self, config: omegaconf.dictconfig.DictConfig, *args: Any, **kwargs: Any) -> None:
        super().__init__(*args, **kwargs)
        self.config = config
        _model = import_module("models.{}".format(config.model_arch))
        _model = getattr(_model, "Model")
        self.model = _model(self.config.loss.lower())
        self.configure_loss()
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=-1):
        ...
        if self.config.loss.lower() == "ocsoftmax":
            pred = self.model(embd_asv_enrol, embd_asv_test, embd_cm_test, labels=label)
            loss = self.loss(pred)
        else:
    ...
    def configure_loss(self):
        ...
        elif self.config.loss.lower() == "cce":
            self.loss = torch.nn.CrossEntropyLoss(
                weight=torch.FloatTensor(self.config.loss_weight)
            )
        elif self.config.loss.lower() == "ocsoftmax":
            self.loss = OCSoftmaxWithLoss()
        else:
            raise NotImplementedError("!")
