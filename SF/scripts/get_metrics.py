...
def main(args):
    ...
    with open(score_file,"r") as f:
        for line in f.readlines(): 
            score,key = line.strip().split()
            scores.append(score)
            keys.append(key)

    sasv_eer, sv_eer, spf_eer=get_all_EERs(np.array(scores).astype("float64"),keys)
    print("SASV: {:.2f}%, SV: {:.2f}%, SPF: {:.2f}%.".format(sasv_eer,sv_eer,spf_eer))

if __name__ == '__main__':
    parser = argparse.ArgumentParser("Make ASVspoof test")
    parser.add_argument("--score_file", required=True, help="The pred score file")
