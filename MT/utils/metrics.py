...
def get_verification_scores(test_data, enroll_data, test_dict, enrol_dict, cm_scores_dict):
    ...
    for utt in test_data:
        ...
        asv_preds.append(cur_score.cpu().data.numpy().item())
        keys.append(target)
    ...
    #     preds.append(cm_preds[i]*sigmoid(asv_preds[i]))
    sasv_eer, sv_eer, spf_eer=get_all_EERs(asv_preds, keys)
    print("SASV EER: {:.2f} SV EER: {:.2f} SPF EER: {:.2f}".format(sasv_eer*100, sv_eer*100, spf_eer*100))

def get_sasv_scores(test_data, sasv_scores_dict):
    """ Computes positive and negative scores given the verification split.
    """
    sasv_preds, keys = [], []
    SUBMISSION_FILE = 'sasv.txt'
    ...
    with open(os.path.join(OUTPUT_DIR, SUBMISSION_FILE),"w") as f:
        for i,utt in enumerate(test_data):
            speaker_id, utt_id, cm_id, target = utt.split()
            index = "test_list_" + str(i)
            sasv_preds.append(sasv_scores_dict[index])
            keys.append(target)
            f.write('%s %s %s %s %f\n'%(speaker_id,
                                        utt_id,
                                        cm_id,
                                        target,
                                        sasv_scores_dict[index],
                                        ))
    ...
    # for i in range(len(asv_preds)):
    #     preds.append(cm_preds[i]*sigmoid(asv_preds[i]))
    sasv_eer, sv_eer, spf_eer=get_all_EERs(sasv_preds, keys)
    print("SASV EER: {:.2f} SV EER: {:.2f} SPF EER: {:.2f}".format(sasv_eer*100, sv_eer*100, spf_eer*100))