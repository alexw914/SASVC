...
class ASP(nn.Module):
    def __init__(self, cat_channel=1536, **kwargs):
        super(ASP, self).__init__()
        self.cat_channel = cat_channel
        self.layer = nn.Conv1d(self.cat_channel, 1536, kernel_size=1)
        self.pooling = AttentiveStatsPool(1536, 128, context=True)
        self.pooling_bn = nn.BatchNorm1d(1536*2)
...
if __name__ =="__main__":
    ...
    model = ResNetSE(layers, num_filters, lin_neurons=256, input_size=40)
    print(summary(model, torch.randn((32,1,40,400)), show_input=False))

    model = ASP(1536)
    print(summary(model, torch.randn((32,1536,25)), show_input=False))
