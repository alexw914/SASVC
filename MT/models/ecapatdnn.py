...
class ECAPA_TDNN(torch.nn.Module):
    ...
    def forward(self, x, lengths=None):
        ...
        # Attentive Statistical Pooling
        x = self.asp(mfa, lengths=lengths)
        x = self.asp_bn(x)
...
class ASP_Linear(nn.Module):
    def __init__(
        ...
        super(ASP_Linear,self).__init__()
        # Attentive Statistical Pooling
        self.asp = AttentiveStatisticsPooling(
            channels[-1],
            attention_channels=attention_channels,
...
if __name__ =="__main__":
    ...
    TDNN = ECAPA_TDNN(input_size=80)
    print(summary(TDNN, torch.randn((64,400,80)), show_input=False))