...
class SEBlock(nn.Module):
    def __init__(self, channel, internal_neurons):
        super(SEBlock, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1) # F_squeeze 
        self.fc = nn.Sequential(
            nn.Linear(channel, internal_neurons, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(internal_neurons, channel, bias=False),
            nn.Sigmoid()
        )
...
if __name__ =="__main__":
    ...
    RepVGG = create_RepVGG_D2se()
    print(summary(RepVGG, torch.randn((32,1,60,400)), show_input=False))